...
/**
 * Adds text lines with proper spacing
 */
...
export const generateSubmissionPDF = async (data: PDFSubmissionData): Promise<void> => {
  try {
    ...
    yPosition = checkPageSpace(pdf, yPosition, 60, data, pageNumber);
    ...
    if (data.notes) {
      yPosition += 5;
      yPosition = checkPageSpace(pdf, yPosition, 30, data, pageNumber);
    ...
    if (data.responses.length > 0) {
      ...
      for (const [blockId, blockResponses] of Object.entries(responsesByBlock)) {
        yPosition = checkPageSpace(pdf, yPosition, 40, data, pageNumber);
        ...
        for (const response of blockResponses) {
          ...
          const questionLines = wrapText(pdf, questionText, maxWidth - 10);
          const questionHeight = questionLines.length * 6 + 15;

          yPosition = checkPageSpace(pdf, yPosition, questionHeight, data, pageNumber);

          // Question border (left side)
          pdf.setDrawColor(36, 92, 79);
          pdf.setLineWidth(1);
          pdf.line(margin, yPosition - 2, margin, yPosition + questionHeight - 10);
