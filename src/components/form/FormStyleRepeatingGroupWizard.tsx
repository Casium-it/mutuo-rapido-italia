...
export function FormStyleRepeatingGroupWizard({
  ...
  const renderQuestionText = () => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      if (formData[placeholderKey]) {
        // Se abbiamo un valore, mostra l'opzione selezionata
        const placeholder = currentQuestion.placeholders[placeholderKey];
        if (placeholder.type === "select") {
          const options = (placeholder as any).options;
          const selectedOption = options.find((opt: any) => opt.id === formData[placeholderKey]);
          parts.push(
          ...
          // Per input o altri tipi
          parts.push(
            <span 
              key={`placeholder-${placeholderKey}`} 
              className="bg-[#F8F4EF] text-[#245C4F] font-medium px-[10px] py-[4px] rounded-[6px] mx-1 my-0.5"
            >
        ...
        // Altrimenti mostra un segnaposto
        parts.push(
          <span
            key={`placeholder-${placeholderKey}`}
            className="bg-[#F8F4EF] text-[#C4BFB8] px-[10px] py-[4px] rounded-[6px] font-medium mx-1 my-0.5"
          >
            ___
          </span>
        );
  ...
  return (
    <div className="max-w-xl animate-fade-in">
      <div className="mb-6">
        <h2 className="text-[18px] md:text-[22px] leading-normal font-semibold text-gray-900 mb-2">
          {renderQuestionText()}
        </h2>
        <Separator className="h-[1px] bg-[#F0EAE0] mt-5 mb-6" />
      </div>
