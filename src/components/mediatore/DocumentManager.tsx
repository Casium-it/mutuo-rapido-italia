...
export function DocumentManager({ submissionId }: DocumentManagerProps) {
  ...
  const fetchPraticaId = async () => {
    try {
      ...
      console.error('Error fetching pratica ID:', error);
      toast.error('Errore nel caricamento della pratica', { duration: 3000 });
      setLoading(false);
    }
  };
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      console.error('Error fetching documents:', error);
      toast.error('Errore nel caricamento dei documenti', { duration: 3000 });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 10 * 1024 * 1024) {
        toast.error('Il file è troppo grande. Dimensione massima: 10MB', { duration: 3000 });
        return;
      }
  ...
  const uploadDocument = async () => {
    ...
    try {
      ...
      toast.success('Documento caricato con successo', { duration: 3000 });
      ...
      toast.error('Errore nel caricamento del documento', { duration: 3000 });
    } finally {
      setUploading(false);
    }
  };
  ...
  const downloadDocument = async (doc: PraticaDocument) => {
    try {
      ...
      toast.error('Errore nel download del documento', { duration: 3000 });
    }
  };
  ...
  const deleteDocument = async (doc: PraticaDocument) => {
    ...
    try {
      ...
      toast.success('Documento eliminato con successo', { duration: 3000 });
      fetchDocuments();
    } catch (error) {
      console.error('Error deleting document:', error);
      toast.error('Errore nell\'eliminazione del documento', { duration: 3000 });
    }
  };
