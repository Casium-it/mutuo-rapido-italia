...
const SimulazioneAvanzata = () => {
  ...
  const startSimulation = async () => {
    ...
    try {
      ...
      if (cachedForm && cachedForm.blocks.length > 0) {
        defaultActiveBlocks = cachedForm.blocks.filter(block => block.default_active).map(block => block.block_id);
      } else {
        // Fallback to static blocks if cache fails
        const {
          allBlocks
        } = await import('@/data/blocks');
        defaultActiveBlocks = allBlocks.filter(block => block.default_active).map(block => block.block_id);
      }
  ...
  return <div className="min-h-screen flex flex-col bg-[#f7f5f2]">
      ...
      <main className="flex-1 px-4 md:px-6 py-8 md:py-12 max-w-3xl mx-auto w-full">
        {/* Welcome message */}
        {slug && <div className="mb-6 p-4 bg-[#245C4F]/10 rounded-lg text-center">
        ...
        <div className="space-y-4">
          ...
          <OptionCard icon={Home} title="Simulazione Mutuo" description="Calcola il tuo mutuo personalizzato" onClick={startSimulation} loading={loading} />
        </div>
        
        {/* Resume simulation link */}
        <div className="mt-8 text-center">
          <button onClick={() => navigate("/riprendi-simulazione")} className="inline-flex items-center gap-2 text-[#245C4F] font-bold hover:underline transition-all">
...
const OptionCard = ({
  ...
  const isMobile = useIsMobile();
  return <div className="flex items-center justify-between p-5 bg-white rounded-[12px] border border-[#BEB8AE] hover:shadow-md transition-all group cursor-pointer shadow-[0_3px_0_0_#AFA89F] hover:shadow-[0_3px_4px_rgba(175,168,159,0.25)]" onClick={onClick}>
      <div className="flex items-center gap-4">
        {!isMobile && <div className="text-gray-600 flex-shrink-0">
      ...
      <div className="bg-[#245C4F] hover:bg-[#1e4f44] p-3 rounded-[10px] transition-colors flex items-center justify-center ml-2 flex-shrink-0 shadow-[0_3px_0_0_#1a453e]">
        {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <ArrowRight className="w-5 h-5 text-white" />}
      </div>
    </div>;
};
